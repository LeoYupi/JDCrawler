...
# 表格标题
TITLE_LABEL = ['商品编号', '商品名称',  '价格', '商家', '商品详情地址']
GOOD_LABEL = ['no', 'name',  'price', 'shop', 'detail_addr']
...
if __name__ == '__main__':
    ...
    excel.save_file('./爬取信息.xlsx')
    df = pd.read_excel('爬取信息.xlsx')
    df=df[['商品名称',   '商家', '商品详情地址','价格']]
    df.sort_values(by='价格', ascending=True, inplace=True)
    df['序号'] = range(1, df.shape[0]+1)
    df.set_index('序号', inplace=True)
    df.to_excel('价格升序.xlsx', index=False)
    print(df)
    shuru=input('请输入整数统计的价格区间：')
    if not shuru.isdigit():
        print("非法字符，程序退出！")
        exit(0)
    qujian=eval(shuru)
    shuzi=0
    while shuzi*qujian<float(df.loc[df.shape[0],'价格']):
        shuzi+=1
    i=0
    quantity={}
    for i in range(shuzi):
        df2 = df.loc[(df['价格'] >= i*qujian)&(df['价格']<=(i+1)*qujian)]
        quantity[str(i*qujian)+'~'+str((i+1)*qujian)]=df2.shape[0]
    print(quantity)
    ha = pd.DataFrame.from_dict(quantity, orient='index', columns=['数量'])
    ha = ha.reset_index().rename(columns={'index': '价格区间'})
    print(ha)
    ax = sns.barplot(x="价格区间", y="数量", data=ha, estimator=np.median, capsize=0.2, errcolor='c')
